...
#include "my_rviz_plugin/my_rviz_panel.hpp"
#include <memory>
#include <string>
...
void MyRvizPanel::subscribeToTopics() {
  // Subscribe to status updates
  sub_status_ = node_->create_subscription<std_msgs::msg::String>(
      "status_updates", 10,
      std::bind(&MyRvizPanel::statusCallback, this, std::placeholders::_1));
}

void MyRvizPanel::statusCallback(const std_msgs::msg::String::SharedPtr msg) {
  last_status_msg_ = msg->data;
}

void MyRvizPanel::updatePanel() {
...
void MyRvizPanel::onButton1Clicked() {
...
void MyRvizPanel::onButton2Clicked() {
...
void MyRvizPanel::publishCommand(const std::string &command) {
  auto msg = std::make_unique<std_msgs::msg::String>();
  msg->data = command;
  pub_command_->publish(std::move(msg));
}

void MyRvizPanel::save(rviz_common::Config config) const {
  Panel::save(config);
...
void MyRvizPanel::load(const rviz_common::Config &config) {
  Panel::load(config);
